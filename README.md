...
*   **🔒 Secure Authentication:** Email-only login flow with strict email verification requirements and built-in password recovery/reset flows.
*   **🔗 SEO & Social Ready:** Built-in Open Graph (OG) and Twitter card support for beautiful link unfurling and social sharing previews.
*   **🎨 Dark Mode UI:** A beautiful, frosted-glass dark theme (#2d313a) designed to be easy on the eyes in dark stage environments.

## 🚀 How it Works

1.  **Sign Up / Login:** Create a free account using your email. You will need to verify your email address before logging in.
...
3.  In your Supabase dashboard, go to **Authentication -> Providers -> Email** and ENSURE **Confirm email** is enabled.
4.  For custom email templates (like password resets) and to prevent emails from going to spam, configure a **Custom SMTP** provider in your Supabase Auth settings.
5.  Create a Storage Bucket named `pdfs`.
6.  Run the following SQL to secure your bucket:
    ```sql
